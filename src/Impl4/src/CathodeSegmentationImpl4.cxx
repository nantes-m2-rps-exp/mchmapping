...
CathodeSegmentation::CathodeSegmentation(
    ...
    : mSegType{segType}, mIsBendingPlane{isBendingPlane},
      mPadGroups{std::move(padGroups)}, mDualSampaIds{getUnique(mPadGroups)},
      mPadGroupTypes{std::move(padGroupTypes)}, mPadSizes{std::move(padSizes)},
      mCatPadIndex2PadGroupIndex{}, mCatPadIndex2PadGroupTypeFastIndex{},
...
std::vector<int>
CathodeSegmentation::getNeighbouringCatPadIndexs(int catPadIndex) const {
...
bool CathodeSegmentation::isValid(int catPadIndex) const {
  return catPadIndex >= 0 &&
         catPadIndex < static_cast<int>(mCatPadIndex2PadGroupIndex.size());
}
